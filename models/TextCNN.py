...
class Config(object):
    ...
    def __init__(self, dataset, embedding, notes):
        ...
        self.train_path = f'{dataset}/train_dataset.csv'
        self.val_path = f'{dataset}/val_dataset.csv'
        self.test_path = f'{dataset}/test_dataset.csv'
        self.class_list = [x.strip() for x in
                           open(f'{dataset}/pre_data/class.txt', encoding='utf-8').readlines()]  # 类别名单
        self.vocab_path = f'{dataset}/pre_data/vocab.pkl'  # 词表
